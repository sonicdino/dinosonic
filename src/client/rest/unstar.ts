...
import { createResponse, database, getFields, getUserByUsername, validateAuth } from '../../util.ts';
    ...
    if (ids?.length) {
        for (const id of ids) {
            if (id.startsWith('t')) {
                ...
                } else userData.starred = undefined;
...
unstar.get('/unstar.view', handleunstar);
unstar.post('/unstar.view', handleunstar);
