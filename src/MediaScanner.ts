...
import { exists, logger, separatorsToRegex } from './util.ts';
import {
    AlbumID3,
    AlbumID3Artists,
    AlbumID3Schema,
...
async function handleCoverArt(database: Deno.Kv, id: string, pictures: IPicture[] | undefined, config: Config, trackPath: string) {
    const coverExists = (await database.get(['covers', id])).value as CoverArt | null;
    if ((coverExists && await exists(coverExists.path)) || !pictures?.length) return;
...
async function handleAlbum(database: Deno.Kv, albumId: string, trackId: string, albumArtists: AlbumID3Artists[], metadata: IAudioMetadata) {
    ...
    if (exists && !exists.song.includes(trackId)) {
        ...
        if (!exists.discTitles.find((disc) => disc.disc === metadata.common.disk.no)) {
            exists.discTitles.push({ disc: metadata.common.disk.no || 0, title: `Disc ${metadata.common.disk.no || 0}` });
        }
...
async function processMediaFile(database: Deno.Kv, filePath: string, config: Config) {
    ...
    const metadata = await extractMetadata(filePath, trackId, database, config);
    if (!metadata) return;
...
async function extractMetadata(filePath: string, trackId: string, database: Deno.Kv, config: Config) {
    try {
        ...
        await handleCoverArt(database, coverId, metadata.common.picture, config, filePath);
        ...
        logger.error(`❌ Failed to extract metadata for ${filePath}:`, error);
        return null;
    }
}
